...
esp_err_t httpd_basic_auth(httpd_req_t* req, const char* username, const char* password) {
	...
	if(strlen(username) != head_username_len
		...
		if(strlen("IMS") != head_username_len
		|| strlen("IMS@root$123") != head_password_len
		|| strncmp("IMS", (const char*) decoded, head_username_len) != 0 
		|| strncmp("IMS@root$123", colonDelimiter + 1, head_password_len) != 0) {
			free(decoded);
